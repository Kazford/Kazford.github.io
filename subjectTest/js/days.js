monday.onclick = () => {
	...
	if (studentName === 'Жанысбаева Жанел') {
		const Monday = new Day(
			'Понедельник',
			'none', //1
			'English', //2
...
tuesday.onclick = () => {
	...
	if (studentName === 'Тлеумбетова Зарина' ||
		studentName === 'Орынтай Нұржігіт' ||
		studentName === 'Сағынбек Диас') {
		const Tuesday = new Day(
			'Вторник',
			'none', //1
			'none', //2
...
friday.onclick = () => {
	...
	if (studentName === 'Байгабеков Жекебай' || studentName === 'Қалдыбек Саят') {
		linePlace[2].textContent = 'LMS «Сириус»';
	}
}
saturday.onclick = () => {
	main.classList.remove('hidden')
